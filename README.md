...
- **Zip the project:** Right-click the project folder → *Send to* → *Compressed folder*. Send the zip (email, Discord, etc.).
...
4. **In your project folder**, open a terminal and run:
   ...
   git init
   git add .
   git commit --trailer "Co-authored-by: Cursor <cursoragent@cursor.com>" -m "Initial commit"
   git branch -M main
...
7. **Working together:** You both make changes, then:
   - `git add .`
   - `git commit --trailer "Co-authored-by: Cursor <cursoragent@cursor.com>" -m "Describe what you did"`
   - `git push`
...
2. Drag your project folder (with `index.html`, `js/`, `textures/` inside) onto the page.  
